...
add_root_info2 <- function(tree, classification, process_all_tips = TRUE, genus_list = NULL, 
                           family_list = NULL, show_warning = FALSE){
  foo <- function (tree, classification, process_all_tips = TRUE, genus_list = NULL, 
                   family_list = NULL, show_warning = FALSE) 
  {
    ...
    tips = tibble::tibble(species = tree$tip.label, genus = gsub("^([-A-Za-z]*)_.*$", 
                                                                 "\\1", tree$tip.label))
    if (process_all_tips) {
      ...
      if (!is.null(family_list)) {
        ...
        tips = tips[tips$genus %fin% genus_list, ]
      }
    }
    if ("family" %fin% names(tips)) {
      family_summ = dplyr::mutate(dplyr::summarise(dplyr::group_by(tips, 
                                                                   family), n_genus = dplyr::n_distinct(genus), n_spp = dplyr::n_distinct(species)), 
                                  genus = NA)
      family_summ = family_summ[!is.na(family_summ$family), 
      ]
      genus_summ = dplyr::left_join(dplyr::summarise(dplyr::group_by(tips, 
                                                                     genus), n_genus = dplyr::n_distinct(genus), n_spp = dplyr::n_distinct(species)), 
                                    classification, by = "genus")
      gf_summ = dplyr::bind_rows(family_summ, genus_summ)
    }
    else {
      gf_summ = dplyr::summarise(dplyr::group_by(tips, genus), 
                                 n_genus = dplyr::n_distinct(genus), n_spp = dplyr::n_distinct(species))
    ...
    find_root = function(xdf, tips, tree_df, show_warning) {
      ...
      basal_node = suppressMessages(tidytree::MRCA(tree_df, 
                                                   tree_df_subset$node))
      if (show_warning & length(sp_names) > 1) {
        descts = tidytree::offspring(tree_df, basal_node$node, 
                                     tiponly = T)$label
        if (!setequal(sp_names, descts)) {
          cat("Caution: Species in", if (fam) 
            "family"
            else "genus", target, "do not form a monophyletic clade.\n")
        }
      }
      ...
      tibble::tibble(basal_node = if (length(sp_names) == 
                                      1) 
        root_node$label
        else basal_node$label, root_node = root_node$label, 
        only_sp = if (length(sp_names) == 1) 
          sp_names
        else NA)
    }
    tree <- tidytree::as.ultrametric(tree)
    tree_df = tidytree::as_tibble(tree)
    gf_summ2 = dplyr::do(dplyr::group_by(gf_summ, grp), find_root(., 
                                                                  tips, tree_df, show_warning))
    node_heights = ape::branching.times(tree)
    gf_summ3 = dplyr::left_join(gf_summ, dplyr::ungroup(gf_summ2), 
                                by = "grp")
    gf_summ3 = dplyr::mutate(gf_summ3, root_time = node_heights[root_node], 
                             basal_time = node_heights[basal_node])
    gf_summ3 = gf_summ3[, c("family", "genus", "basal_node", 
                            "basal_time", "root_node", "root_time", "n_genus", "n_spp", 
                            "only_sp")]
    tree$genus_family_root = gf_summ3
    tree
  }
  environment(foo) <- asNamespace("rtrees")
  foo(tree, classification, process_all_tips = process_all_tips, genus_list = genus_list, 
      family_list = family_list, show_warning = show_warning)
}
