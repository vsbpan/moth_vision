...
import_COCO <- function(x){
  ...
  if("num_keypoints" %in% names(out$annotations)){
  ...
  tab <- table(out$annotations$image_id)
  out$images$num_annotations <- as.vector(tab)[match(as.character(out$images$id), names(tab))]
...
split_COCO <- function(x, test, val = 0){
  ...
  s_test <- sample(s_test, round(n * test), replace = FALSE)
