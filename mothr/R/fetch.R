...
fetch_path_metadata <- function(database_path = paste(pkgload::pkg_path(vmisc::fake_pkg()), 
                                                      "assets/path_metadata.csv", sep = "/")){
  database <- try(suppressMessages(readr::read_csv(database_path, 
                                                   progress = FALSE)))
  if(!isTRUE(is.data.frame(database))){
    cli::cli_abort("Cannot find path metadata database! Expected path: {.file {database_path}}.")
  }
