...
thin <- function (object, thin = 3, method = c("drop", "mean", "min", 
                                       "max", "mad", "var", "all", "any", "sum")) 
{
  ...
  if (method == "drop") {
    ...
    method <- switch(method, sum = "sum", var = "var", mad = "mad", 
                     mean = "mean", all = "all", any = "any", min = "min", 
                     max = "max")
    if (is.matrix(object)) {
      object <- object[seq_len(n.x * thin), seq_len(n.y * 
                                                      thin), drop = FALSE]
    }
    else {
      object <- object[seq_len(n.x * thin), seq_len(n.y * 
    ...
    group_code <- as.factor(paste(rep(x.index, tot.len/x.new.len), 
                                  rep(rep(Rfast::Sort.int(rep(y.index, x.new.len)), 
                                          z.index.len), col.index.len), rep(Rfast::Sort.int(rep(z.index, 
                                                                                                x.new.len * y.new.len)), col.index.len), Rfast::Sort.int(rep(col.index, 
                                                                                                                                                             tot.len/col.index.len)), sep = "-"))
    val <- Rfast::group(c(object), as.numeric(group_code), 
                        method = method)
    o <- do.call("rbind", strsplit(levels(group_code), "-")) %>% 
      apply(2, as.numeric) %>% as.data.frame() %>% with(order(V4, 
                                                              V3, V2, V1))
    if (is.matrix(object)) {
      out <- matrix(val[o], nrow = n.x, ncol = n.y)
    }
    else {
      out <- array(val[o], dim = c(n.x, n.y, z.index.len, 
                                   col.index.len))
      out <- switch(class(object)[1], cimg = as.cimg(out), 
                    pixset = as.pixset(out), array = as.array(out))
    }
  }
